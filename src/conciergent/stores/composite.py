...
class CompositeStore(Store):
    ...
    async def load_history(self, principal: str) -> list[typing.Any]:
        return await self._messages.load_history(principal)

    async def append_history(self, principal: str, messages: list[typing.Any], *, ttl_seconds: int) -> None:
        await self._messages.append_history(principal, messages, ttl_seconds=ttl_seconds)

    async def replace_history(self, principal: str, messages: list[typing.Any], *, ttl_seconds: int) -> None:
        await self._messages.replace_history(principal, messages, ttl_seconds=ttl_seconds)
    ...
    async def park_approval(
        ...
        await self._messages.park_approval(principal, state, ttl_seconds=ttl_seconds)

    async def take_approval(self, principal: str) -> dict[str, typing.Any] | None:
        return await self._messages.take_approval(principal)
