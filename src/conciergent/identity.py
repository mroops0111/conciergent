"""Stable per-user identity across chat surfaces."""
...
def make_principal(surface: ChatSurface | str, *parts: str) -> str:
    ...
    For example ``make_principal(ChatSurface.slack, team_id, user_id)`` yields
    ``'slack:T1:U1'``.
    """
...
def parse_principal(principal: str) -> tuple[str, tuple[str, ...]]:
    """Split a principal back into its surface and remaining parts."""
